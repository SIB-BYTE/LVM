...
cpu_t *new_cpu(qword *instructions, qword instruction_length, qword stack_size)
{
	...
	// Stack creation:
	cpu->integer_stack = calloc(stack_size, sizeof(qword));
	cpu->stack_capacity = stack_size;
	cpu->sp = -1;

	cpu->fp_stack = calloc(stack_size, sizeof(f64));
	cpu->fp_stack_capacity = stack_size;
	cpu->fp_sp = -1;
...
void execute(cpu_t *cpu)
{
...
#ifdef DEBUG
	printf("%sInstruction pointer -> (%ld)\nStack pointer -> {%d}Floating point stack pointer -> {%d}\n", PUR, cpu->ip, cpu->sp, cpu->fp_sp);
	...
	switch(opcode)
	{
		...
		case IMUL_IMM: cpu->registers[cpu->dst] *= (signed) cpu->src; cpu->ip += 3; break;
		case IDIV_IMM: cpu->registers[cpu->dst] /= (signed) cpu->src; cpu->ip += 3; break;
		...
		case DIV_REG:  cpu->registers[cpu->dst] /= cpu->registers[cpu->src]; cpu->ip += 3; break;
		case IMUL_REG: cpu->registers[cpu->dst] *= (signed) cpu->registers[cpu->src]; cpu->ip += 3; break;
		case IDIV_REG: cpu->registers[cpu->dst] /= (signed) cpu->registers[cpu->src]; cpu->ip += 3; break;
		case POP_REG:  cpu->registers[cpu->src] = peek(cpu); pop(cpu); cpu->ip += 2; break;
		...
		case PUSHF_IMM:  fp_push(cpu, cpu->src); cpu->ip += 2; break;

		// Floating-point registers instructions:
		case MOVSS_REG:  cpu->floating_registers[cpu->dst] =  cpu->floating_registers[cpu->src]; cpu->ip += 3; break;
		case ADDSS_REG:  cpu->floating_registers[cpu->dst] += cpu->floating_registers[cpu->src]; cpu->ip += 3; break;
		case SUBSS_REG:  cpu->floating_registers[cpu->dst] -= cpu->floating_registers[cpu->src]; cpu->ip += 3; break;
		case MULSS_REG:  cpu->floating_registers[cpu->dst] *= cpu->floating_registers[cpu->src]; cpu->ip += 3; break;
		case DIVSS_REG:  cpu->floating_registers[cpu->dst] /= cpu->floating_registers[cpu->src]; cpu->ip += 3; break;
		case IMULSS_REG: cpu->floating_registers[cpu->dst] *= (signed) cpu->floating_registers[cpu->src]; cpu->ip += 3; break;
		case IDIVSS_REG: cpu->floating_registers[cpu->dst] /= (signed) cpu->floating_registers[cpu->src]; cpu->ip += 3; break;
		case PUSHF_REG:  fp_push(cpu, cpu->floating_registers[cpu->src]); cpu->ip += 2; break;
		case POPF_REG:   cpu->floating_registers[cpu->src] = fp_peek(cpu); fp_pop(cpu); cpu->ip += 2; break;
...
const char *bind_name(qword instruction)
{
	switch(instruction)
	{
	  ...
	  case PUSHF_REG:  return "PUSHF_REG";
